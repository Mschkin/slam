    ...
    for (int blocky = 0; blocky < sqrtlength; blocky++)
    {
        for (int y = 0; y < sqrtlength; y++)
        {
            ...
            for (int blockx = (blocky - off_diagonal_number < 0) ? 0 : blocky - off_diagonal_number; blockx < sqrtlength && (blockx < blocky + off_diagonal_number + 1); blockx++)
            {
                for (int x = 0; x < sqrtlength; x++)
                {
                    r_x(blocky, y) -= Hnd_R(blockx, x, blocky, y) * r_y(blockx, x);
                }
            }
            r_x(blocky, y) = (r_x(blocky, y) - L_x(blocky, y)) / Hdx_R(blocky, y);
        }
    }
    free(Hdx_R);
    free(Hdy_R);
...
#define dinterest_dstraight(l, m, i, j, k) dinterest_dstraight[(l) * (2 * off_diagonal_number - 1) * sqrtlength * sqrtlength * 9 + (m)*sqrtlength * sqrtlength * 9 + (i)*sqrtlength * 9 + (j)*9 + (k)]
...
#define full_din_dstraight(e, l, m, i, j, k) full_din_dstraight[(e)*sqrtlength * sqrtlength * 9 * big_block_size + (l) * (2 * off_diagonal_number + 1) * sqrtlength * sqrtlength * 9 + (m)*sqrtlength * sqrtlength * 9 + (i)*sqrtlength * 9 + (j)*9 + (k)]

    for (int e = 0; e < example_indices; e++)
    {
        for (int i = 0; i < sqrtlength * sqrtlength * 9 * block_size; i++)
        {
            dinterest_dstraight[i] = 0;
        }
        for (int i = 0; i < sqrtlength; i++)
        {
            for (int j = 0; j < sqrtlength; j++)
            {
                pure_phase(e, i, j) = (off_diagonal_number <= i) * (i < sqrtlength - off_diagonal_number) * (off_diagonal_number <= j) * (j < sqrtlength - off_diagonal_number);
                norm(i, j) = 0;
                for (int k = 0; k < 9; k++)
                {
                    norm(i, j) += straight(e, i, j, k);
                }
                norm(i, j) = 1 / norm(i, j);
                for (int k = 0; k < 9; k++)
                {
                    for (int l = 0; l < 9; l++)
                    {
                        dnormed_straight_dstraight(i, j, k, l) = -straight(e, i, j, k) * norm(i, j) * norm(i, j);
                    }
                    dnormed_straight_dstraight(i, j, k, k) += norm(i, j);
                    straight(e, i, j, k) *= norm(i, j);
                }
                /* 1<i<N-1; 1<j<N-1
        ...
        for (int pro_range = 0; pro_range < off_diagonal_number; pro_range++)
        {
            for (int straight_x = off_diagonal_number - pro_range; straight_x < sqrtlength - off_diagonal_number + pro_range; straight_x++)
            {
                for (int straight_y = off_diagonal_number - pro_range; straight_y < sqrtlength - off_diagonal_number + pro_range; straight_y++)
                {
                    ...
                    {
                        dummy_pure_phase(straight_y + straight_d / 3 - 1, straight_x + straight_d % 3 - 1) += straight(e, straight_y, straight_x, straight_d) * pure_phase(e, straight_y, straight_x);
                        ...
                        for (int block_y = off_diagonal_number - pro_range + max1; block_y < off_diagonal_number + pro_range - 1 - max1; block_y++)
                        {
                            for (int block_x = off_diagonal_number - pro_range + max1; block_x < off_diagonal_number + pro_range - 1 - max1; block_x++)
                            {
                                ...
                                {
                                    dummy_block(block_y + direction / 3 - 1, block_x + direction % 3 - 1) += straight(e, straight_y + straight_d / 3 - 1 - off_diagonal_number + 1 + block_y, straight_x + straight_d % 3 - 1 - off_diagonal_number + 1 + block_x, direction) * dinterest_dstraight(block_y, block_x, straight_y, straight_x, straight_d);
                                }
                            }
                        }
                        dummy_block(off_diagonal_number - 1, off_diagonal_number - 1) += pure_phase(e, straight_y, straight_x);
                        for (int block_copy = 0; block_copy < block_size; block_copy++)
                        {
                            dinterest_dstraight(0, block_copy, straight_y, straight_x, straight_d) = dummy_block[block_copy];
            ...
            {
                pure_phase(e, 0, set_zero) = dummy_pure_phase[set_zero];
                dummy_pure_phase[set_zero] = 0;
            }
        }
        //np.einsum('ijkl,mnijk->mnijl',dnormed_straight_dstraight,dintered_dstraight)
                ...
                for (int l = 0; l < 9; l++)
                {
                    for (int m = 0; m < 2 * off_diagonal_number - 1; m++)
                    {
                        for (int n = 0; n < 2 * off_diagonal_number - 1; n++)
                        {
                            for (int k = 0; k < 9; k++)
                            {
                                full_din_dstraight(e, m + k / 3, n + k % 3, i, j, l) += dnormed_straight_dstraight(i, j, k, l) * dinterest_dstraight(m, n, i, j, k);
                            }
                        }
                    }
                }
